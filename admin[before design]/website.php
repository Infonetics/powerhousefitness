      ...
      if(isset($_POST["btn_web"])) {
        ...
        $_FILES["image"]= $_FILES["website_image"];
         ...
         if($response['type'] == 'error')   {   
              ...
              $q1="UPDATE `manage_website` SET `title`='".trim($title)."',`website_name`='".trim($website_name)."',`logo`='".trim($website_logo)."',`currency_code`= '".trim($currency_code)."',`contact`= '".trim($contact)."',`email`='".trim($email)."',`whatsapp_no`='".trim($whatsapp_no)."',`facebook_link`='".trim($facebook_link)."',`instagram_link`='".trim($instagram_link)."',`open_hours`='".trim($open_hours)."',`open_hours1`='".trim($open_hours1)."',`address`='".trim($address)."',`happy_customers`='".trim($happy_customers)."',`perfect_bodies`='".trim($perfect_bodies)."',`working_hours`='".trim($working_hours)."',`success_stories`='".trim($success_stories)."'";
