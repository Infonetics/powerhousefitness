...
<link href="assets/js/plugins/@fortawesome/fontawesome-free/css/all.min.css" rel="stylesheet" />
                  ...
                  <div>
                    <?php include("include/config.php");
                         $query = "SELECT * FROM `tbl_customer_pt` cpt 
                          left join `tbl_customer` c on(c.`customer_id`=cpt.`customer_id`)
                          left join `tbl_pt_payment` ptp on(ptp.`pt_payment_customer_pt_id`=cpt.`customer_pt_id`)
                          left join `tbl_total` t on(t.`mp_customer_membership_id`=cpt.`customer_pt_id`)
                          WHERE cpt.`customer_pt_id` = '".$_SESSION['customer_pt_id']."' ORDER BY `pt_payment_id` DESC"; 
                  ...
                  <div class="col-sm-6">
                    ...
                    <div class="row pl-4 f-14">
                      ...
                      <div class="col-sm-6">
                        <label><?php echo $row['pt_payment_type'];?></label>
                      </div>
                    </div>
                  </div>
                </div>
                  <div class="table-responsive p-3">
                      ...
                      <tbody>
                        <?php 
                         $query = "SELECT * FROM `tbl_customer_pt` cpt 
                          left join `tbl_membership` m on(m.`membership_id`=cpt.`membership_id`)
                          left join `tbl_total` t on(t.`customer_pt_id`=cpt.`customer_pt_id`)
                          WHERE cpt.`customer_pt_id` = '".$_SESSION['customer_pt_id']."' "; 
                          $result = $con->query($query);  
                          $data=array();$totals=array();
                          $row = mysqli_fetch_array($result);
                                $data['plan_name']        = $row['name'];
                                $data['cm_starting_date'] = $row['customer_pt_starting_date'];
                                $data['cm_expiry_date']   = $row['customer_pt_expiry_date'];
                                $data['price']            = $row['price'];
                          $query="SELECT * FROM `tbl_total` WHERE `customer_pt_id`= '".$_SESSION['customer_pt_id']."' ORDER BY `sort_order` ASC "; 
                          $result = $con->query($query); 
