        ...
        <div class="card-content">
          <div class="table-responsive">
            <table class="table table-bordered table-dark">
              ...
              <tbody>
                <?php 
                $sql = "SELECT *  FROM `".DB_PREFIX."`.`tbl_customer` c LEFT JOIN `tbl_customer_membership` cm ON(c.`customer_id`=cm.`customer_id` and cm.`customer_membership_status`=1) LEFT JOIN `tbl_customer_pt` cpt ON(c.`customer_id`=cpt.`customer_id` and cpt.`customer_pt_status`=1) ORDER BY c.`customer_id` DESC LIMIT 0,5";
