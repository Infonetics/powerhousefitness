  ...
  <div class="main-content">
    <nav class="navbar navbar-top navbar-expand-md navbar-dark" id="navbar-main">
      <div class="container-fluid">
        <!-- Brand -->
        <a class="h4 mb-0 text-white text-uppercase d-none d-lg-inline-block" href="index.php">Membership History</a>
    <!-- Header -->
        ...
        <div class="col">
          <div class="card shadow">
            <div class="card-header bg-transparent">
              <h3 class="mb-0">Membership History</h3>
            </div>
            <div class="card-body">
                <!-- ------- -->
              <div class="card">
                    ...
                    <table class="table table-flush dataTable" id="datatable-basic" role="grid" aria-describedby="datatable-basic_info">
                      <thead class="thead-light">
                        <tr role="row">
                          ...
                          <th class="sorting" tabindex="0" aria-controls="datatable-basic" rowspan="1" colspan="1" aria-label="Position: activate to sort column ascending" >Membership Plan</th>
                      ...
                      <tbody>
                        <?php 
                         /* $sql = "SELECT * FROM tbl_customer_membership cm
                                  ...
                                  where cm.`cm_plan`=1 AND `title_text` LIKE 'Grand Total' GROUP BY tot.`mp_customer_membership_id` ORDER BY `receipt_no` DESC";*/

                                  $sql = "SELECT * FROM `".DB_PREFIX."`.`tbl_customer` c
                                  left join `".DB_PREFIX."`.`tbl_customer_membership` cmm on(cmm.`customer_id`=c.`customer_id`)
                                  left join `".DB_PREFIX."`.`tbl_membership` m on(m.`membership_id`=cmm.`membership_id`)
                                  left join `".DB_PREFIX."`.`tbl_total` tot on(tot.`mp_customer_membership_id`=cmm.`customer_membership_id`)
                                  where cmm.`cm_plan`= 1 AND `title_text` LIKE 'Grand Total' ORDER BY `receipt_no` DESC";
                                  // echo $sql;exit;
                        ...
                        <tr>
                          ...
                          <td><?php if($row['cm_plan'] == 1){echo "Membership Plan";}else {echo "PT Plan";}?></td>
                          <td><?php echo $row['name'];?></td>
                          <td><?php echo $row['customer_membership_starting_date'];?></td>
                          <td><?php echo $row['customer_membership_expiry_date']; ?></td>
                          <td><?php echo $row['total_amount']; ?></td>
                          <td><?php echo $row['total_payment_date']; ?></td>
                        </tr>
                      <?php } ?>
                    </tbody>
                    </table>
